...
export class DES extends BlockCipher {
    ...
    reset() {
        ...
        for (let i = 0; i < 56; i++) {
            ...
            keyBits[i] = this._key.words[keyBitPos >>> 5] >>> 31 - keyBitPos % 32 & 1;
        }
        this._subKeys = [];
        for (let nSubKey = 0; nSubKey < 16; nSubKey++) {
            this._subKeys[nSubKey] = [];
            const bitShift = BIT_SHIFTS[nSubKey];
            for (let i = 0; i < 24; i++) {
                this._subKeys[nSubKey][i / 6 | 0] |= keyBits[(PC2[i] - 1 + bitShift) % 28] << 31 - i % 6;
                this._subKeys[nSubKey][4 + (i / 6 | 0)] |= keyBits[28 + (PC2[i + 24] - 1 + bitShift) % 28] << 31 - i % 6;
            }
            this._subKeys[nSubKey][0] = this._subKeys[nSubKey][0] << 1 | this._subKeys[nSubKey][0] >>> 31;
            for (let i = 1; i < 7; i++) {
                this._subKeys[nSubKey][i] = this._subKeys[nSubKey][i] >>> (i - 1) * 4 + 3;
            }
            this._subKeys[nSubKey][7] = this._subKeys[nSubKey][7] << 5 | this._subKeys[nSubKey][7] >>> 27;
        }
        this._invSubKeys = [];
        for (let i = 0; i < 16; i++) {
            this._invSubKeys[i] = this._subKeys[15 - i];
        }
    }

    protected _doCryptBlock(m: number[], offset: number, subKeys: number[][]): void {
        ...
        for (let round = 0; round < 16; round++) {
            ...
            let f = 0;
            for (let i = 0; i < 8; i++) {
                f |= SBOX_P[i][((this._rBlock ^ subKey[i]) & SBOX_MASK[i]) >>> 0];
            }
            [this._lBlock, this._rBlock] = [this._rBlock, this._lBlock ^ f];
        }

        [this._lBlock, this._rBlock] = [this._rBlock, this._lBlock];
